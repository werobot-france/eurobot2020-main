...
class Blue3(API):
  ...
  def run(self):
    ...
    self.navigation.goTo(x=160, y=775, speed=60, threshold=7)
    self.navigation.orientTo(theta=pi/2, speed=50)
    self.navigation.orientTo(theta=pi/2, speed=30)
    
    # routine de préhension
    input('>Confirm?')
    ...
    self.navigation.goTo(x=self.positionWatcher.getX()-14, y=940, speed=60, backward=True)
...
class Blue3(API):
  ...
  def run(self):
    ...
    input('>Confirm?')
    ## 2 - ECEUIL COMMUN VERT
    self.navigation.goTo(x=300, y=600, speed=60)
    self.navigation.goTo(x=170, y=775, speed=60, threshold=7)
    self.navigation.orientTo(theta=pi/2, speed=50)
    self.navigation.orientTo(theta=pi/2, speed=30)
